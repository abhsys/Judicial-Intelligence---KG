...
@router.get("/indiankanoon/search")
def indiankanoon_search(
    request: Request,
    q: str = Query(min_length=1),
    limit: int = Query(default=10, ge=1, le=10),
) -> dict[str, Any]:
    service = request.app.state.indiankanoon_service
    keyword = q.strip()
    rows = service.search(keyword, limit=limit)
    data = [
        {
            "rank": int(item.rank),
            "title": item.title,
    ...
    return {"query": keyword, "count": len(data), "data": data}
